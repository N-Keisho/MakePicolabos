    ...
    private void OnTriggerEnter(Collider collider){
        if (collider.gameObject.tag == "Player"){
            ...
            if (this.gameObject.tag == "Box"){
                ...
                effectNum.makeEffectNum("+1000", this.gameObject.transform.position, "Box");
            }
            else if (this.gameObject.tag == "Rock"){
                scoreManager.score -= 50;
                if (scoreManager.timer >= 6f) scoreManager.timer -= 5f;
                scoreManager.rocks += 1;
                effectNum.makeEffectNum("-50", this.gameObject.transform.position, "Rock");
            }
            else{
                scoreManager.score += 10;
                scoreManager.parts += 1;
                effectNum.makeEffectNum("+10", this.gameObject.transform.position, "Parts");
            }
            Destroy(gameObject);
        }
    }
